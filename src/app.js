...
function downloadPng(name, id) {
	svg.saveSvgAsPng(document.getElementById(id), name + '.png', {
		...
		fetch({
			...
			if (result.length == 0) {
				...
				fetchData().then(response => {
					...
					if (
						result[0].datalink.inline_data.visualization
					...
					let metric_x =
						result[0].datalink.inline_data.visualization.x_axis;
					let metric_y =
						result[0].datalink.inline_data.visualization.y_axis;
					// append those metrics as div attributes, so that they cna be used later
					document
						.getElementById(divid)
						.setAttribute('metric_x', metric_x);
					document
						.getElementById(divid)
						.setAttribute('metric_y', metric_y);
					join_all_json(
						result,
						divid,
						metric_x,
